...
- **Use Universal Links (iOS) / App Links (Android)** where possible — they require domain verification and are harder to spoof than custom URI schemes.
...
For high-security apps, implement SSL certificate pinning to prevent man-in-the-middle attacks on rooted/jailbroken devices. Without pinning, a device-level CA cert installed by the user (or attacker) can intercept all traffic.

Note: certificate pinning adds maintenance overhead (must update on certificate rotation). Use only when the threat model warrants it.
