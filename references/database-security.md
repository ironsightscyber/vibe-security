...
**Scale of the problem:** CVE-2025-48757 (May 2025) documented 303 endpoints across 170 Lovable-generated apps where tables were fully readable via an unauthenticated `?select=*&limit=1000` call with only the public `anon` key. Confirmed exposed data included names, emails, Google Maps API keys, Gemini API keys, eBay tokens, financial records, and user credentials.
